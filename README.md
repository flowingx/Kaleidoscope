...
- **矢量形状工具**:
  ...
  - **形状库**: 可直接在画布上创建 `三角形`、`星形`、`圆形`、`方形`、`菱形`、`六边形`、`箭头`、`十字`、`心形` 等矢量图形。
  - **点选与套索**: 选择工具提供 `Point` 和 `Lasso` 两种模式，可单点编辑一个形状，也可圈选多个形状后批量移动、缩放和删除。
  - **Repeat / Free**: 每个形状都可以选择是否参与万花筒映射；`Free` 形状只会保留在放置位置，适合做标注、贴纸和笔记式元素。
  - **属性面板**: 选中形状后，可在面板中调整大小、旋转角度和映射模式。

- **强大的对称引擎**:

  - **Lens 视角预设**: 提供 `Calm`、`Bloom`、`Prism` 三种观看角度，一键切换切片数量、镜像方式、画面角度和动态效果，并带有短暂过渡动画。
...
选择工具中的 `Point` 用于单个形状，`Lasso` 用于圈选多个形状。Shape Library 中的 `Repeat` 会让形状进入万花筒映射，`Free` 则像笔记软件里的贴纸一样只放在一个位置。

关于下一阶段“万花筒作为观看角度”的设计方向，请参见 [docs/vision.md](docs/vision.md)。
