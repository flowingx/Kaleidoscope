...
class App:
    ...
    def run(self):
        while self.is_running:
            ...
            time_delta_ms = time_delta_seconds * 1000
            self._handle_events()
    ...
    def _handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            if not is_input_blocked:
                self._handle_drawing_events(event)
    ...
    def _handle_drawing_events(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            # First, check if the click was on a custom UI element (like the color picker)
            color_event = self.ui_handler.handle_color_picker_click(event.pos)
            if color_event:
                if color_event['type'] == 'select':
                    self.active_color_selection = color_event['target']
                elif color_event['type'] == 'pick':
                    if self.active_color_selection == 'start':
                        self.start_color = color_event['color']
                    else:
                        self.end_color = color_event['color']
            # If not on a custom UI, check if it's on the drawing canvas
    ...
    def _handle_gui_events(self, event):
        ...
        if event.type == pygame_gui.UI_HORIZONTAL_SLIDER_MOVED:
            if event.ui_element == self.ui_handler.elements['brush_size_slider']:
                self.brush_size = self._calculate_and_update_brush_size(event.value)
            # 处理引导线滑块的移动
            elif event.ui_element == self.ui_handler.elements['guides_slider']:
                slider_value = int(event.value)
                # 将滑块的档位 (0, 1, 2, 3) 映射到切片数量 (0, 8, 12, 16)
                mapping = {0: 0, 1: 8, 2: 12, 3: 16}
                self.guide_line_slices = mapping.get(slider_value, 0)
                
                # 更新标签以显示当前档位
                label_text = f"Guides: {self.guide_line_slices if self.guide_line_slices > 0 else 'Off'}"
                self.ui_handler.elements['guides_value_label'].set_text(label_text)
        ...
        if len(self.user_path) > 1:
            if self.brush_type == 'Line':
                ...
                for p in self.user_path:
                    pygame.draw.circle(self.screen, config.PREVIEW_LINE_COLOR, p['pos'], int(p['size']/2)+1, 1)
    ...
    def _finish_current_animation(self):
        ...
        drawing.draw_on_surface(self.layers[0].surface, remaining_elements, self.num_slices, self.symmetry_mode, 0, 1.0)
